...
Ttem45::Ttem45() : predstr( NUMTEM )
{
  ...
  string tem_diagfile = "tem_diag.txt";
  temdiagfile.open( tem_diagfile.c_str(), ios::out );
...
// Ecosystem water pools determined by the integrator***********
  ...
  predstr.at( I_SMLL ) = "SMOISTLL";
  
  // volumetric soil moisture lower layer
  predstr.at( I_VSMLL ) = "VSMLL";
...
// Other ecosystem water pools ******************************
  ...
  predstr.at( I_AVLWLL ) = "AVAILH2OLL";  // plant available soil water
...
//    }
  ...
  soil.setAVLH2OLL( pstate[I_SMLL] - soil.getWILTPT() );
  if( soil.getAVLH2OLL() < ZERO )
  {
    soil.setAVLH2OLL( ZERO );
  }

  soil.setVSM( pstate[I_VSM] );
  soil.setVSMLL( pstate[I_VSMLL] );
  soil.setSWP(pstate[I_VSMLL]);
  soil.setSWPLL(soil.getSWP() );
  soil.setSWP(pstate[I_VSM]);
  soil.setMOIST( pstate[I_SM] );
  soil.setMOISTLL( pstate[I_SMLL] );
  soil.setKH2O( pstate[I_VSMLL], moistlim );
  soil.setKH2OLL( soil.getKH2O() );
...
//  temdiagfile << "LAI = in cropdynamics = " << veg.getLAI() << endl;
...
//    prob = 1.0 - exp(-0.005*atms.getPREC() );
    prob = 1.0 - exp(-0.01*atms.getPREC() );
    if( prob < ZERO ) { prob = ZERO; }
...
//    if(atms.getPREC() == 0)
//    {
//      surf_wetness =  0.65 * surf_wetness;
//    }
    veg.pen.setR_SS( exp(8.206 - 4.205*surf_wetness) ); // using formulation from SiB2
  #endif
    ...
    surf_wetness = 100.0*soil.getVSMLL()/soil.getPCTPOR();
    if( surf_wetness > 1.0 ) { surf_wetness = 1.0; }
    if( surf_wetness < ZERO ) { surf_wetness = ZERO; }
    veg.pen.setR_SSLL(exp(8.206 - 4.205*sqrt(surf_wetness*prob)) ); // using formulation from SiB2
  #endif
...
nopen = 0;
  ...
  microbe.updateDynamics( ag.cmnt,
                          ...
                          pstate[I_SOLC],
                          pstate[I_SOLN],
                          pstate[I_SM] + pstate[I_SMLL],
                          pstate[I_VSM] + pstate[I_VSMLL],
                          pstate[I_AVLN],
                          moistlim,
                          ag.tillflag,
                          ag.getTILLFACTOR( ag.cmnt ),
                          soil.getKH2O() + soil.getKH2OLL(),
                          veg.getRLTRC(),
                          nopen,
                          ag.getIMMBADD(),
                          ag.getVOLAC(),
 ...
 if((soil.getAVLH2O() < .5 * soil.getAWCAPMM()) && (veg.getPESOILW() > 0.5 * atms.getPREC()))
{
  ...
  soil.setINEET(veg.getPET());
  soil.setEET(soil.getINEET());
  soil.setINEETLL(veg.getPETLL());
  soil.setEETLL(soil.getINEETLL());
...
/*   soil.updateHydrology(elev,
  ...
  soil.infilt( pstate[I_SM] );
  soil.runu( atms.getRAIN(),
             pstate[I_SM],
             pstate[I_VSM]);
  soil.runl( pstate[I_SMLL] );
...
//  cout << "LEAFC = " << pstate[I_LEAFC] << " " << pdstate[I_LEAFC] << " " << veg.getALLOCLC() << " " << veg.getRMLEAF() << " " << veg.getLTRLC() << " " << pstate[I_LABILEC] <<  endl;
  ...
  pdstate[I_LABILEC] = veg.getGPP()
                  ...
                  - veg.getALLOCSEEDC()
                  - veg.getRGRWTH();
// cout << "LabileC = " << pstate[I_LABILEC] << " " << pdstate[I_LABILEC] << " " << veg.getGPP() << " " << veg.getALLOCLC() << " " <<  veg.getALLOCSC() << " " << veg.getALLOCRC() << " " << veg.getALLOCSEEDC() << " " << veg.getRGRWTH() << endl;
...
/*  pdstate[I_SM] = soil.getSNOWINF()
  ...
  pdstate[I_SMLL] = soil.getINFILTRATE()
                    - soil.getEETLL()
                    - soil.getRUNOFFB();
...
//  cout << "delta sm = " << pdstate[I_SM] << " " << pstate[I_SM] << " " << atms.getRAIN() << "  " << soil.getEET() << " " << soil.getRPERC() <<  " " << soil.getSPERC() << endl;
  ...
  pdstate[I_VSM] = pdstate[I_SM]/(0.5 * 1000.0);
  ...
  pdstate[I_VSMLL] = pdstate[I_SMLL]/(soil.getROOTZ() * 1000.0);
//cout << " delta VSMLL = " << pdstate[I_VSMLL] <<" delta SM = " << pdstate[I_SMLL] << " root zone = " << soil.getROOTZ() << endl;
  if( pstate[I_VSMLL]+pdstate[I_VSMLL] <= ZERO )
  {
    pdstate[I_VSMLL] = 0.001 - pstate[I_VSMLL];
  }
...
//  cout << "initializeState = " << ag.cmnt << " " << y[I_LABILEC] << endl;
}
...
//  temdiagfile << "LAI = in natdynamics = " << veg.getLAI() << endl;
//  if(pdm == 0) {cout << "lai first = " << veg.getLAI() << " " << pdm << endl;}
...
// cout << "entering stepmonth " << ag.state << endl;
...
//        temdiagfile << "year, day = " << pdyr << " " << pdm << " " << atms.getTAIRD() << endl;
//       cout << "year, day = " << pdyr << " " << pdm << " " << atms.getTAIRD() << endl;
...
//  if( (1 == ag.state) && ((ag.getGROWDD() >= ag.getGDDHARVST(ag.cmnt)) || (harcnt == 0 && pdm == 9))    && (0 == ag.getFROSTFLAG() ) )
  if( (1 == ag.state) && ((ag.getGROWDD() >= ag.getGDDHARVST(ag.cmnt)) || (harcnt == 0 && pdm == 274))    && (0 == ag.getFROSTFLAG() ) )
//  if( (1 == ag.state) && (ag.getGROWDD() >= ag.getGDDHARVST(ag.cmnt)) && (0 == ag.getFROSTFLAG() ) )
  {
//    cout << "harvest = " << pdm << endl;
...
//  if(dm == 0) {cout << "lai = " << veg.getLAI() << " " << dm << endl;}
