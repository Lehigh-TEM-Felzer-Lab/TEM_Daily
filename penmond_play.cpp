...
*/
  ...
  double throughfall;  // precipitation throughfall
...
//  ecanmmmo = (1.0 - exp(-0.1*precip))
//   BSF  Adapt daily to monthly
//  ecanmmmo = (1.0 - exp(-50.0*precip))
//  ecanmmmo = (1.0 - exp(-0.4*precip))
//                      *(1.0 - exp(-kext[pdcmnt]*lai))*maxh2ocanopy*lai;
  
//  BSF new canopy interception based on Helvey and Patric 1965
...
{
if(pdcmnt == 4 || pdcmnt == 5 || pdcmnt == 6 || pdcmnt == 10 || pdcmnt == 11 || pdcmnt == 12 ) { 
  if(lai > 0.5) {
  throughfall = 0.901*precip - 0.031;
  }
  else
  {
  throughfall = 0.914*precip - 0.015;
  }
}
else if (pdcmnt == 7)
{
if(lai < 0.5 && dm < 153 ) {
  throughfall = precip*(9.46*log(precip) + 62.32)/100.0; 
}
else if(lai > 0.5 ) {
  throughfall = precip*(15.67*log(precip) + 21.75)/100.0;
}
else if(lai < 1.0 && dm > 245 ) {
  throughfall = precip*(4.36*log(precip) + 25.97)/100.0;
}
}
else if (pdcmnt == 8)
{
  ...
  if(throughfall < 0.0) { throughfall = 0.0;}
  if(throughfall > precip) { throughfall = precip;}
//  ecanmmmo =  (precip - throughfall)*(1.0 - exp(-kext[pdcmnt]*lai));
  ecanmmmo =  (precip - throughfall);
//  ecanmmmo = 0.25 * precip * (1 - expl(-kext[pdcmnt]*lai));
  ecanmmmo = 0.13 * precip * (1 - expl(-kext[pdcmnt]*lai));
...
//cout << "rn = " << nirr << " " << totalbedo << " " << lwcor << " " << esoilmmmo << endl;
...
//cout << "diag = " << etsw << " " << wc << " " << pmc << " " << ws << " " << pms << endl;
